...
void getInitialEnergy() {
  for (int i = 0; i < numNodes; i++)
    initialEnergy += emp[i]->GetTotalEnergyConsumption();
  std::cout << "Initial Energy (@" << Simulator::Now().GetSeconds() << ") = " << initialEnergy << std::endl;
}
...
void PacketSinkTraceSink(Ptr<const Packet> packet, const Address & from) {
  ...
  std::cout << "Packet of size " << packet->GetSize() << "B received at " 
      ...
      << (int)bulkAppSink1->GetTotalRx() << std::endl;  
  std::cout << "Total energy consumed = " << energyConsumed << std::endl;
  throughput = (int)bulkAppSink1->GetTotalRx() * 8.0 / 1000.0 / (Simulator::Now().GetSeconds() - startTime);
  std::cout << "Throughput = " << throughput << "kbps" << std::endl;
  std::cout << "Throughput per hop = " << (int)bulkAppSink1->GetTotalRx() * 8.0 * numHops / 1000.0 / (Simulator::Now().GetSeconds() - startTime) << "kbps" << std::endl;
  if (useCheckPoints) {
    if ((int) bulkAppSink1->GetTotalRx() >= checkPoint) {
  ...
  if ((int) bulkAppSink1->GetTotalRx() >= fileSize) {
    if (writeInFile) {
      if (!myFile.is_open()) {
...
int main(int argc, char * argv[])
{
  ...
  BulkSendHelper bulkApp("ns3::TcpSocketFactory", Inet6SocketAddress(deviceInterfaces.GetAddress(numNodes - 1,1), 9));
  bulkApp.SetAttribute("MaxBytes", UintegerValue(fileSize));
  ApplicationContainer sourceApps = bulkApp.Install(c.Get(0));
  sourceApps.Start(Seconds(0.0));
  sourceApps.Stop(Seconds(10000.0));

  PacketSinkHelper bulkAppSink("ns3::TcpSocketFactory", Inet6SocketAddress(Ipv6Address::GetAny(), 9));

  ApplicationContainer sinkApps = bulkAppSink.Install(c.Get(numNodes - 1));

  sinkApps.Start(Seconds(0.0));
  sinkApps.Stop(Seconds(10000.0));

  // Tracing
  bulkAppSink1 = DynamicCast<PacketSink>(sinkApps.Get(0)); // get sink
  ...
  Simulator::Schedule (Seconds(startTime + 0.02), &getInitialEnergy);
