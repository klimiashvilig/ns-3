...
/*static void GetTotalEnergyConsumption (std::string context, double oldValue, double newValue)
{
  std::cout << context << " TotalEnergyConsumption: " << newValue
                       << " from " << oldValue << " at " << Simulator::Now().GetSeconds() << "\n";
}*/
...
int main(int argc, char * argv[])
{
  ...
  for (int i = 0; i < numNodes; i++) {
    ...
    //std::cout << std::string("phy" + std::to_string(i)) << std::endl;
    //es[i]->TraceConnect ("RemainingEnergy", std::string ("phy" + std::to_string(i)), MakeCallback (&GetTotalEnergyConsumption));
  }
