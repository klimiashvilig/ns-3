...
std::string fileName = "802-15-4-my-mac-results-" + (varDistance? (std::to_string(fileSize) + "B") : (std::to_string(distance) + "m")) + ".txt";
...
int main(int argc, char * argv[])
{
  ...
  fileName = "802-15-4-my-mac-results-" + (varDistance? (std::to_string(fileSize) + "B") : (std::to_string(distance) + "m")) + ".txt";
  ...
  BulkSendHelper onOff("ns3::TcpSocketFactory", Inet6SocketAddress(deviceInterfaces.GetAddress(numNodes - 1,1), 9));
  ...
  PacketSinkHelper onOffSink("ns3::TcpSocketFactory", Inet6SocketAddress(Ipv6Address::GetAny(), 9));
