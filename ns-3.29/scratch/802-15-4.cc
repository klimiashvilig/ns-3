...
void PacketSinkTraceSink(Ptr<const Packet> packet, const Address & from) {
  ...
  std::cout << "Throughput = " << (int)onOffSink1->GetTotalRx() * 8.0 / 1000.0 / (Simulator::Now().GetSeconds() - startTime) << "kbps" << std::endl;
...
int main(int argc, char * argv[])
{
  ...
  if (this_logging)
    std::cout << "Creating protocol stack at the nodes\n";
  InternetStackHelper internetv6;
  internetv6.Install (c);

  if (this_logging)
    std::cout << "Installing protocol stack at the nodes\n";
  SixLowPanHelper sixlowpan;
  // sixlowpan.SetDeviceAttribute ("UseMeshUnder", BooleanValue (true) );
  NetDeviceContainer devices = sixlowpan.Install (lrwpanDevices); 
 
  if (this_logging)
    std::cout << "Create networks and assign IPv6 Addresses.\n";
