...
static const int defaultFileSize = 200;
int fileSize;
int received;

std::ofstream myFile;
std::string fileName = "LoRaresults-" + std::to_string(distance) + "B.txt";
...
int main (int argc, char *argv[])
{
    if (writeInFile)
        myFile.open(fileName, std::ofstream::app);
    for (fileSize = (variableFileSize ? 512:defaultFileSize); fileSize <= (variableFileSize ? 20480:defaultFileSize); fileSize += 512) {
        CommandLine cmd;
        cmd.Parse (argc, argv);
