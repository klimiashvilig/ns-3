...
std::string fileName = "wifiresults-" + std::to_string(fileSize) + "B-3flows" + (routing ? "" : "-nr") + ".txt";
...
int main(int argc, char * argv[]) {
  ...
  fileName = "wifiresults-" + std::to_string(fileSize) + "B-3flows" + (routing ? "" : "-nr") + ".txt";
  ...
  if (numNodes >= 6) {
    ...
    std::cout << "Sender #1: " << "x - " << x << " y - " << y << std::endl;
    x = c.Get(receiverNode - 1)->GetObject<MobilityModel>()->GetPosition().x;
    y = c.Get(receiverNode - 1)->GetObject<MobilityModel>()->GetPosition().y;
    std::cout << "Receiver #1: " << "x - " << x << " y - " << y << std::endl;
    x = c.Get(senderNode + 2)->GetObject<MobilityModel>()->GetPosition().x;
    y = c.Get(senderNode + 2)->GetObject<MobilityModel>()->GetPosition().y;
    std::cout << "Sender #2: " << "x - " << x << " y - " << y << std::endl;
    x = c.Get(receiverNode - 2)->GetObject<MobilityModel>()->GetPosition().x;
    y = c.Get(receiverNode - 2)->GetObject<MobilityModel>()->GetPosition().y;
    std::cout << "Receiver #2: " << "x - " << x << " y - " << y << std::endl;
    double secondFlowOffset;
    double thirdFlowOffset;
    if (distance == 75) {
      secondFlowOffset = r->GetValue(0, 0.5);
      thirdFlowOffset = r->GetValue(0, 0.5);
    }
    else if (distance == 150) {
      secondFlowOffset = r->GetValue(1, 1.5);
      thirdFlowOffset = r->GetValue(1, 1.5);
    }
    else {
      secondFlowOffset = r->GetValue(2, 4);
      thirdFlowOffset = r->GetValue(2, 4);
    }
    std::cout << "secondFlowOffset = " << secondFlowOffset << std::endl;
    std::cout << "thirdFlowOffset = " << thirdFlowOffset << std::endl;
    ...
    thirdFlowSourceApps.Start (Seconds (thirdFlowOffset + (routing? 0.0:20.0)));
    ...
    thirdFlowSinkApps.Start (Seconds (thirdFlowOffset + (routing? 0.0:20.0)));
