...
std::string fileName = "wifiresults-" + std::to_string(fileSize) + "B-random-4flows-TC-" + std::to_string((int)TCInterval) + ".txt";
...
int main (int argc, char *argv[])
{
  ...
  for (int distance = (variableDistance ? 75:defaultDistance); distance <= (variableDistance ? 450:defaultDistance); distance += 75) {
    int numNodes = (distance * distance / 10) / 500 + 8; // area = distance * distance / 10
    if (distance == 75)
      numNodes = 8;
