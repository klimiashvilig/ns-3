...
static const int defaultDistance = 225;
...
std::string fileName = "wifiresults-" + std::to_string(fileSize) + "B-PS-1472B" + (routing? "":"-nr") + ".txt";
  ...
  for (int distance = (variableDistance ? 75:defaultDistance); distance <= (variableDistance ? 600:defaultDistance); distance += distanceBetweenNodes) {
    ...
    for (int runNum = (variableRunNum ? 1:defaultRunNum); runNum <= (variableRunNum ? 30:defaultRunNum); runNum++) {
      ...
      OnOffHelper onOff ("ns3::UdpSocketFactory",
                         InetSocketAddress (i[2].GetAddress (1), 9));
