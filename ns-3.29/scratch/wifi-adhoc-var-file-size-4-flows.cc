...
static const int distance = 75;
static const int defaultFileSize = 10000;
static int fileSize;
...
std::string fileName = "wifiresults-" + std::to_string(distance) + "m-random-4flows-TC-" + std::to_string((int)TCInterval) + ".txt";
...
int main (int argc, char *argv[])
{
  ...
  for (fileSize = (variableFileSize ? 50:defaultFileSize); fileSize <= (variableFileSize ? 10000:defaultFileSize); fileSize += ((fileSize < 1000) ? 50:200)) {
    ...
    for (int runNum = 1; runNum <= (variableRunNum ? 15:1); runNum++) {
      ...
      if (numNodes >= 8) {
        ...
        OnOffHelper fourthFlowOnOff("ns3::UdpSocketFactory",
                         InetSocketAddress (i.GetAddress (receiverNode - 3), 9));
        fourthFlowOnOff.SetConstantRate(DataRate("54Mbps"));
        ApplicationContainer fourthFlowSourceApps = fourthFlowOnOff.Install (c.Get (senderNode + 3));
        fourthFlowSourceApps.Start (Seconds (0.0));
        fourthFlowSourceApps.Stop (Seconds (10000.0));

        PacketSinkHelper fourthFlowSink ("ns3::UdpSocketFactory",
                               InetSocketAddress (Ipv4Address::GetAny (), 9));

        ApplicationContainer fourthFlowSinkApps = fourthFlowSink.Install (c.Get (receiverNode - 3));

        fourthFlowSinkApps.Start (Seconds (0.0));
        fourthFlowSinkApps.Stop (Seconds (10000.0));
      }
