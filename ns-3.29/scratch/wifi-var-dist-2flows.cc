...
double timeBeforeSending = 0;
double interPacketInterval = 2;
double startTime = 15;
...
void getEnergy() {
  ...
  timeBeforeSending = Simulator::Now().GetSeconds();
...
void PacketSinkTraceSink(Ptr<const Packet> packet, const Address & from) {
  if ((int) sink1->GetTotalRx() >= fileSize) {
    ...
    if (writeInFile) {
      ...
      myFile << Simulator::Now().GetSeconds() - timeBeforeSending << "," << energyConsumed << ",";
      myFile.close();
    }
    return;
  }
...
int main(int argc, char * argv[]) {
  ...
  ////////////////////////////////////// Create Application Layer Between Source and Sink /////////////////////////////////////////////
  ...
  apps.Stop (Seconds (numPackets * interPacketInterval + (routing ? 0 : startTime)));
     ...
     ////////////////////////////////////// Create Application Layer Between OTHER Source and Sink /////////////////////////////////////////////
  ...
  if (numNodes >= 4) {
    ...
    double secondFlowInterval = r->GetValue(0, 0.5);
    if (distance == 75)
    ...
    std::cout << "secondFlowInterval = " << secondFlowInterval << std::endl;

    // UDP connection from N0 to "receiverNode" - 1
    Address sinkAddress (InetSocketAddress (i.GetAddress(receiverNode - 1), sinkPort)); // interface of n24
    PacketSinkHelper sink ("ns3::UdpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), sinkPort));
    ApplicationContainer sinkApps = sink.Install (c.Get(receiverNode-1)); //receivernode-1 as sink
    sinkApps.Start (Seconds (secondFlowOffset - 1 + (routing ? 0 : startTime)));
    sinkApps.Stop (Seconds (numPackets * interPacketInterval + (routing ? 0 : startTime)));
    Ptr<Socket> ns3UdpSocket = Socket::CreateSocket (c.Get (0), UdpSocketFactory::GetTypeId ()); //source at n0
    ...
    UdpClientHelper client (sinkAddress, sinkPort);
    client.SetAttribute ("MaxPackets", UintegerValue (100));
    client.SetAttribute ("Interval", TimeValue (Seconds (secondFlowInterval)));
    client.SetAttribute ("PacketSize", UintegerValue (512));
    ApplicationContainer apps ;
    apps = client.Install (c.Get (1));
    apps.Start (Seconds (secondFlowOffset - 1 + (routing ? 0 : startTime)));
    apps.Stop (Seconds (numPackets * interPacketInterval + (routing ? 0 : startTime)));
  }
  ...
  Simulator::Schedule(Seconds(startTime), & getEnergy);
